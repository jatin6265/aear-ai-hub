-- Approval Review Modal backend: detailed review payload, decision workflow, and short-lived execution tokens.
...
BEGIN
  ...
  ELSIF v_user_role IN ('owner', 'admin') THEN
    v_raci_confirmation := format('You are Accountable as %s for %s → %s', initcap(v_user_role), v_request.resource, v_request.action);
  ELSE
    v_raci_confirmation := format('Your role (%s) is not Accountable for %s → %s', v_user_role, v_request.resource, v_request.action);
  END IF;
...
BEGIN
  ...
  IF v_decision = 'more_info' THEN
    ...
    VALUES (
      ...
      'pending',
      jsonb_build_object('request_id', p_request_id, 'message', v_reason)
    );
...
GRANT EXECUTE ON FUNCTION public.get_approval_review_payload(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.submit_approval_review_decision(uuid, text, text) TO authenticated;
